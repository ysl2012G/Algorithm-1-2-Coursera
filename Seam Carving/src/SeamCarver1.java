...
public class SeamCarver {
	...
	private Color[][] createPictureArray(Picture picture) {
		int rowNum = picture.height();
		int colNum = picture.width();
		Color[][] pictureArray = new Color[colNum][rowNum];
	...
	private Picture createPicture() {
		...
		for (int col = 0; col < colNum; col++) {
			for (int row = 0; row < rowNum; row++) {
				pict.set(col, row, pictArray[col][row]);
			}
		}
		return pict;
	}
	...
	private double deltaX(int col, int row) {
		...
		Color c1, c2;
		if (isChanged) {
			c1 = pictArray[rowindex][colindex - 1];
			c2 = pictArray[rowindex][colindex + 1];
		} else {
			c1 = pictArray[colindex - 1][rowindex];
			c2 = pictArray[colindex + 1][rowindex];
		}
	...
	private double deltaY(int col, int row) {
		...
		Color c1, c2;
		if (isChanged) {
			c1 = pictArray[rowindex + 1][colindex];
			c2 = pictArray[rowindex - 1][colindex];
		} else {
			c1 = pictArray[colindex][rowindex + 1];
			c2 = pictArray[colindex][rowindex - 1];
	...
	private void transposePicture() {
		int rowNum = getHeight();
		int colNum = getWidth();
		Color[][] tempPict = new Color[rowNum][colNum];
		for (int col = 0; col < colNum; col++) {
			for (int row = 0; row < rowNum; row++) {
	...
	private void removeSeam(int[] seam) {
		int rowNum = getHeight();
		int colNum = getWidth();
		Color[][] tempPict = new Color[colNum][rowNum - 1];
		for (int col = 0; col < colNum; col++) {
			int index = seam[col];
